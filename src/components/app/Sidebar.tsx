...
export function Sidebar({ plan = 'free', userName, onClose }: SidebarProps) {
  ...
  return (
    <aside className="flex h-full w-64 flex-col bg-background border-r border-border">
      {/* Logo */}
        ...
        {NAV_ITEMS.map(({ label, href, icon: Icon, paidOnly }) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-sans font-medium transition-colors',
                active
                  ? 'border-l-2 border-primary bg-surface text-primary pl-[10px]'
                  : 'text-muted-foreground hover:bg-surface hover:text-foreground border-l-2 border-transparent pl-[10px]'
              )}
            >
        ...
        {isFree ? (
          <div className="rounded-lg bg-surface border border-border p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-mono text-[10px] font-medium text-primary uppercase tracking-wider">Plano Free</span>
            </div>
            <p className="font-sans text-xs text-muted-foreground mb-3 leading-snug">
              3 briefs por mês. Faça upgrade para ilimitado.
            </p>
            <Link
              href="/dashboard/settings?upgrade=1"
              className={cn(buttonVariants({ size: 'sm' }), 'w-full justify-between text-xs font-sans')}
            >
              Fazer upgrade
          ...
          <div className="rounded-lg bg-surface border border-border p-4">
            ...
            {userName && (
              <p className="mt-1 font-sans text-xs text-muted-foreground truncate">{userName}</p>
            )}
          </div>
        )}
      </div>
